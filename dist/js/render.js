...
function renderGame(ctx) {
  ...
  // scale wrapper: characters render CHAR_SCALE× around their ground anchor
  const charDraw = (wx, wy, fn) => () => {
  ...
  for (const c of GORE.corpses) {
    ...
    draws.push({ d: c.x + c.y - 0.3, fn: charDraw(c.x, c.y, () => drawCorpse(ctx, c)) });
  }
  for (const pk of STATE.pickups)
    draws.push({ d: pk.x + pk.y, fn: () => drawPickup(ctx, pk) });
  for (const e of STATE.enemies) {
    if (e.dead || e.x < minX - 1 || e.x > maxX + 2 || e.y < minY - 1 || e.y > maxY + 2) continue;
    draws.push({ d: e.x + e.y, fn: charDraw(e.x, e.y, () => drawEnemy(ctx, e)) });
  }
  if (!p.dead) draws.push({ d: p.x + p.y, fn: charDraw(p.x, p.y, () => drawHero(ctx, p)) });
  if (STATE.pet && typeof drawPet === 'function')
    draws.push({ d: STATE.pet.x + STATE.pet.y, fn: charDraw(STATE.pet.x, STATE.pet.y, () => drawPet(ctx, STATE.pet)) });
  ...
  // execute draws defensively: one bad sprite must never blank the frame
  // or leak alpha/composite/transform state into later sprites
  for (const d of draws) {
    ctx.save();
    try { d.fn(); } catch (err) { if (!STATE._drawErr) { STATE._drawErr = 1; console.error('draw error:', err); } }
    ctx.restore();
  }
