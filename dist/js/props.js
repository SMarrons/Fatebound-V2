...
function drawPickup(ctx, pk) {
  ...
  switch (pk.kind) {
    ...
    case 'item': {
      ...
      const leg = pk.item.rarity === 'legendary';
      const beamH = leg ? 64 : 46;
      if (leg) {
        const pulse = 0.7 + Math.sin(STATE.time * 4 + pk.x) * 0.3;
        const gg = ctx.createRadialGradient(s.x, s.y - 3, 0, s.x, s.y - 3, 22 * pulse);
        gg.addColorStop(0, 'rgba(255,53,42,0.5)'); gg.addColorStop(1, 'rgba(0,0,0,0)');
        ctx.fillStyle = gg; ctx.fillRect(s.x - 24, s.y - 28, 48, 48);
      }
      const g = ctx.createLinearGradient(s.x, s.y - beamH, s.x, s.y);
      g.addColorStop(0, 'rgba(0,0,0,0)');
      g.addColorStop(1, c + (leg ? 'cc' : 'aa'));
      ctx.fillStyle = g;
      ctx.fillRect(s.x - (leg ? 3.5 : 2.5), s.y - beamH, leg ? 7 : 5, beamH);
      if (leg) { ctx.shadowColor = c; ctx.shadowBlur = 10; }
      ctx.fillStyle = c;
      ctx.save(); ctx.translate(s.x, s.y - 8 - bob); ctx.rotate(Math.PI / 4);
