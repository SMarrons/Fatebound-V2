...
function drawHero(ctx, p) {
  ...
  const bootC = p.equipment.boots ? '#3f464e' : '#3a2e21';
  const legC = '#33291d';
  ...
  // ---- torso ----
  const torsoC = hasChest ? '#79828c' : '#6b4f35';
  const tY = gy - bob;
  ...
  if (hasChest) {
    ...
    for (const side of [-1, 1]) {
      ctx.fillStyle = vGrad(ctx, tY - 27, tY - 17, '#5d6772', 24, -12);
  ...
  if (hasHelm) {
    ...
    ctx.fillStyle = vGrad(ctx, hY - 8.4, hY + 2, '#7e868e', 28, -16);
...
function drawHeroWeapon(ctx, s, ang, w, p, bob) {
  ...
  if (rare) { ctx.shadowColor = rc; ctx.shadowBlur = 7; }
  switch (type) {
    case 'sword':
      grip(-1, 3, 2.8);
      blade(4.5, 21, 3.2);
